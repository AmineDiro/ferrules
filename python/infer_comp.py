...
if __name__ == "__main__":
    ...
    # Storage for results
    results = {batch_size: {'batch_throughputs': [], 'batch_latencies': [],
                            'single_throughputs': [], 'single_latencies': []} 
    ...
    for batch_size in batch_sizes:
        ...
        for N in N_values:
            _, throughput, latency = measure_inference_batch(batch_session, batch_size, repeats=N)
            results[batch_size]['batch_throughputs'].append(throughput)
            results[batch_size]['batch_latencies'].append(latency)

            _, throughput_s, latency_s = measure_inference_single(single_session, batch_size, repeats=N)
    ...
    for batch_size in batch_sizes:
        plt.plot(N_values, results[batch_size]['batch_throughputs'], label=f"Batched Model (batch={batch_size})")
        plt.plot(N_values, results[batch_size]['single_throughputs'], '--', label=f"Single-Inference (batch={batch_size})")
    ...
    plt.figure(figsize=(12, 8))
    for batch_size in batch_sizes:
        plt.plot(N_values, results[batch_size]['batch_latencies'], label=f"Batched Model (batch={batch_size})")
        plt.plot(N_values, results[batch_size]['single_latencies'], '--', label=f"Single-Inference (batch={batch_size})")
